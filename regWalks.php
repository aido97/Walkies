...
$persons7 = ("SELECT requestID, user_id, first_name, mon, tue,wed,thu, fri, sat, sun,pickup, dropoff, addr1, addr2, zip, phone_number, price, profile_image_url, walker_id FROM walkies_web.regular_walks WHERE walk_status = 'Y' AND user_id != '$user_id';");
			 ...
			 <!-- Start of new Profile Container -->
                    ...
                    <!--PHP Function below that uses the $person / $result query
            	    	...
            	    	foreach($result7 as $row){
            	    	    ...
            	    	    $weekdays = '';
            	    	if ($row[mon] == 'Y') {$weekdays .= "Mon, ";}
            	    	if ($row[tue] == 'Y') {$weekdays .= "Tue, ";}
            	    	if ($row[wed] == 'Y') {$weekdays .= "Wed, ";}
            	    	if ($row[thu] == 'Y') {$weekdays .= "Thu, ";}
            	    	if ($row[fri] == 'Y') {$weekdays .= "Fri, ";}
            	    	if ($row[sat] == 'Y') {$weekdays .= "Sat, ";}
            	    	if ($row[sun] == 'Y') {$weekdays .= "Sun.";} 
					    ...
					    $line14 =   '</div>';
                        $iterator++;
                        $table = $line1. '' .$line2. '' .$line3. '' .$line4. '' .$line5. '' .$line6. '' .$line7. '' .$line8. '' .$line9. '' .$line10. '' .$line11. '' .$line12. '' .$line13.''.$line14;
                                           ...
                                           if(isset($_POST['submit_Btn1']))
                  {
                    ...
                    if($_POST[$i] == "yes"){
                            $sql = "UPDATE walkies_web.regular_walks SET regular_walks.walk_status='N' WHERE requestid = '$row1[requestid]'";
                	        $sql1 ="UPDATE walkies_web.regular_walks SET regular_walks.walker_id = '$user_id' where requestid = '$row1[requestid]'";
